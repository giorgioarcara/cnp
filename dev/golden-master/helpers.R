# Helpers for comparing the in-development package's behavior against any
# committed reference point (tag, branch, or commit SHA) - "golden master" /
...
#' Does a file exist at a given git ref?
#'
#' Used to auto-detect which historical file layout applies at a given ref
#' (e.g. whether a reorganization commit has happened yet), so the same
#' comparison script can target any point in history.
#'
#' @param ref Character. A git tag, branch, or commit-ish ref.
...
#' @return `TRUE`/`FALSE`.
path_exists_at_ref = function(ref, path, repo_root = git_repo_root()){
  status = suppressWarnings(system2("git", c("-C", repo_root, "cat-file", "-e", paste0(ref, ":", path)),
                                     stdout = FALSE, stderr = FALSE))
  identical(status, 0L)
}

#' Read a file's content as it existed at a given git ref
#'
...
read_file_at_ref = function(ref, path, repo_root = git_repo_root()){
  out = suppressWarnings(system2("git", c("-C", repo_root, "show", paste0(ref, ":", path)),
                                  stdout = TRUE, stderr = TRUE))
  status = attr(out, "status")
  if (!is.null(status) && status != 0){
    stop("Could not read '", path, "' at ref '", ref, "'. git output:\n", paste(out, collapse = "\n"))
  }
  out
}

#' Source one or more files, as they existed at a given ref, into one fresh environment
#'
...
build_old_env = function(ref, paths, repo_root = git_repo_root()){
  env = new.env(parent = globalenv())
  for (path in paths){
    code = read_file_at_ref(ref, path, repo_root)
